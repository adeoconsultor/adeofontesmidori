...
*/
User Function PCGRFEMAI(_lEnvMail,_cNumPed)
...
Default _lEnvMail := .T. //Se envia PDF por E-mail.
...
Private cGrupo := ""
/*      
Private _aCol := {	0035,; //01 - Item | 
    	          	0120,; //02 - Codigo | 
				    0240,; //03 - Descricao do Material|     
				    0970,; //04 - Grupo| 
				    1050,; //05 - UM| 
					1120,; //06 - Quantidade| 
					1270,; //07 - Valor Unitario| 
					1470,; //08 - IPI| 
					1560,; //09 - Valor Total| 
					1810,; //10 - Entrega| 
					1980,; //11 - S.C.| 
					2160} //Posicao final ajuste
*/
Private _aColNew := {{0035,0085,0,0},; //01 - Item | 
	    	         {0120,0120,0,0},; //02 - Codigo | 
					 {0240,0730,0,0},; //03 - Descricao do Material|     
					 {0970,0085,0,0},; //04 - Grupo| 
					 {1055,0070,0,0},; //05 - UM| 
					 {1125,0150,1,0},; //06 - Quantidade| 
					 {1275,0200,1,0},; //07 - Valor Unitario| 
					 {1475,0090,1,0},; //08 - IPI| 
					 {1555,0250,1,0},; //09 - Valor Total| 
					 {1815,0170,1,0},; //10 - Entrega| 
					 {2005,0180,0,0},; //11 - S.C.| 
					 {2165,0000,1,0}} //Posicao final ajuste
...
While (_cAlias)->(!Eof())
	...
	If _nCont >= _ItemPG .Or. cPedidoAtu <> cPedidoAnt // Quebra paginas por qtde itens
    ...
    AADD(_aItens, { Alltrim((_cAlias)->C7_ITEM) ,;	                                     //01 - Item //Item | 
					Alltrim((_cAlias)->C7_PRODUTO) ,;						             //02 - codigo//Codigo | 
					Substr(Alltrim((_cAlias)->B1_DESC),1,39) ,;							 //03 - descricao //Descricao do Material|     
					Alltrim( (_cAlias)->B1_GRUPO )  ,;                                   //04 - Grupo| 
					Alltrim( (_cAlias)->C7_UM ) ,;								         //05 - unidade de medida //UM| 
					Alltrim(Transform((_cAlias)->C7_QUANT,"@E 9,999,999.99" ))  ,;	     //06 - Quantidade //Quantidade| 
					Alltrim(Transform((_cAlias)->C7_PRECO, "@E 9,999,999.9999" ))  ,;	 //07 - VLR UNIT //Valor Unitario| 
					Alltrim(Transform((_cAlias)->C7_IPI,"@E 99.99" ))  ,;		         //08 - IPI //IPI| 
 					Alltrim(Transform((_cAlias)->C7_TOTAL,"@E 999,999,999.99"  )) ,;	 //09 - VLR TOT //Valor Total|
					DTOC((_cAlias)->C7_DATPRF) ,;							             //10 - data de entrega //Entrega| 
					Alltrim( ( _cAlias)->C7_NUMSC ) })                                   //11 - S.C.| 
                       
	If (Len( Alltrim((_cAlias)->B1_DESC )) > 39)  
	    AADD(_aItens, { "" ,;	                             //01 - Item //Item | 
						"" ,;						         //02 - codigo//Codigo | 
						Substr((_cAlias)->B1_DESC,40,70) ,;	 //03 - descricao //Descricao do Material|     
						"" ,;                                //04 - Grupo| 
						"" ,;								 //05 - unidade de medida //UM| 
						"" ,;	                             //06 - Quantidade //Quantidade| 
						"" ,;	                             //07 - VLR UNIT //Valor Unitario| 
						"" ,;			                     //08 - IPI //IPI| 
						"" ,;	                             //09 - VLR TOT //Valor Total| 
						"" ,;							     //10 - data de entrega //Entrega| 
						"" })                                //11 - S.C.| 
	...
	For X:= 1 To Len(_aItens)
		...
		For Y:= 1 To Len(_aAux)           
			...
			oPrn:SayAlign(nLin,_aColNew[Y][01],_aAux[Y],oFontCORI, _aColNew[Y][02] ,200 , , _aColNew[Y][03], _aColNew[Y][04] )  
		Next Y
	Next X
...
/*
//nLin := 485//450
oPrn:say (nLin,_aCol[01],"ITEM",oFontCORI)                  //01 - Item| 
oPrn:say (nLin,_aCol[02],"COD.",oFontCORI)                  //02 - Codigo| 
oPrn:say (nLin,_aCol[03],"DESCRICAO DO MATERIAL",oFontCORI) //03 - Descricao do Material|
oPrn:say (nLin,_aCol[04],"GRP",oFontCORI)                   //04 - Grupo| 
oPrn:say (nLin,_aCol[05],"UM",oFontCORI)                    //05 - UM| 
oPrn:say (nLin,_aCol[06],"QTDE",oFontCORI)                  //06 - Quantidade| 
oPrn:say (nLin,_aCol[07],"VLR UNIT",oFontCORI)              //07 - Valor Unitario| 
oPrn:say (nLin,_aCol[08],"IPI",oFontCORI)                   //08 - IPI| 
oPrn:say (nLin,_aCol[09],"VLR TOTAL",oFontCORI)             //09 - Valor Total| 
oPrn:say (nLin,_aCol[10],"ENTREGA",oFontCORI)               //10 - Entrega| 
oPrn:say (nLin,_aCol[11],"S.C",oFontCORI)                   //11 - C.C. |
*/

oPrn:SayAlign(nLin,_aColNew[01][01],"ITEM"                 ,oFontCORI, _aColNew[01][02] ,200 , , _aColNew[01][03], _aColNew[01][04] ) //01 - Item| 
oPrn:SayAlign(nLin,_aColNew[02][01],"COD."                 ,oFontCORI, _aColNew[02][02] ,200 , , _aColNew[02][03], _aColNew[02][04] ) //02 - Codigo| 
oPrn:SayAlign(nLin,_aColNew[03][01],"DESCRICAO DO MATERIAL",oFontCORI, _aColNew[03][02] ,200 , , _aColNew[03][03], _aColNew[03][04] ) //03 - Descricao do Material|
oPrn:SayAlign(nLin,_aColNew[04][01],"GRP"                  ,oFontCORI, _aColNew[04][02] ,200 , , _aColNew[04][03], _aColNew[04][04] ) //04 - Grupo| 
oPrn:SayAlign(nLin,_aColNew[05][01],"UM"                   ,oFontCORI, _aColNew[05][02] ,200 , , _aColNew[05][03], _aColNew[05][04] ) //05 - UM| 
oPrn:SayAlign(nLin,_aColNew[06][01],"QTDE"                 ,oFontCORI, _aColNew[06][02] ,200 , , _aColNew[06][03], _aColNew[06][04] ) //06 - Quantidade| 
oPrn:SayAlign(nLin,_aColNew[07][01],"VLR UNIT"             ,oFontCORI, _aColNew[07][02] ,200 , , _aColNew[07][03], _aColNew[07][04] ) //07 - Valor Unitario| 
oPrn:SayAlign(nLin,_aColNew[08][01],"IPI"                  ,oFontCORI, _aColNew[08][02] ,200 , , _aColNew[08][03], _aColNew[08][04] ) //08 - IPI| 
oPrn:SayAlign(nLin,_aColNew[09][01],"VLR TOTAL"            ,oFontCORI, _aColNew[09][02] ,200 , , _aColNew[09][03], _aColNew[09][04] ) //09 - Valor Total| 
oPrn:SayAlign(nLin,_aColNew[10][01],"ENTREGA"              ,oFontCORI, _aColNew[10][02] ,200 , , _aColNew[10][03], _aColNew[10][04] ) //10 - Entrega| 
oPrn:SayAlign(nLin,_aColNew[11][01],"S.C"                  ,oFontCORI, _aColNew[11][02] ,200 , , _aColNew[11][03], _aColNew[11][04] ) //11 - C.C. |
...
oPrn:say(_nLin01,1450,"MOEDA:",oFontRODN)   
_nLin01 += 30                                       

_cMoeda := "" 
Do Case
   Case SC7->C7_MOEDA == 1
        _cMoeda := StrZero(SC7->C7_MOEDA,1)+ " - ( R$ - REAL )" 
   Case SC7->C7_MOEDA == 2
        _cMoeda := StrZero(SC7->C7_MOEDA,1)+ " - ( US$ - DOLAR )"
   Case nOpc == 3
        _cMoeda := StrZero(SC7->C7_MOEDA,1)+ " - ( UFIR - UFIR )"
   Case nOpc == 4
        _cMoeda := StrZero(SC7->C7_MOEDA,1)+ " - ( EUR - EURO )"
   Case nOpc == 5
        _cMoeda := StrZero(SC7->C7_MOEDA,1)+ " - ( JPY - IENE )"        
   Otherwise
        // Otherwise é opcional.
		_cMoeda := ""
EndCase
 
oPrn:SayAlign( _nLin01,1450, _cMoeda ,oFontROD, (2375 - 1450 ),200 , , 2, 0 )
_nLin01 += 30                                       
_nLin01 += 30                                       
/*     
...
Default cTo      := " "    // Defina e?mail para destinatario
