...
User Function XmlNfeSef310(cTipo,cSerie,cNota,cClieFor,cLoja,cNotaOri,cSerieOri)
...
Local cCodlan		:= ""
Local cMunTransp	:= ""
Local cValLiqS		:= SuperGetMv("MV_BR10925", ,"2")		//Adicionado por Willer tratar retenções de venda
Local cCliTakat		:= SuperGetMv("MA_RETTAKA", ,"000008")		//Adicionado por Willer tratar retenções de venda
...
Local nValIpiBene	:= 0
Local nValtrib	    := 0
Local cCodTemp   	:= "" // Variável criada por Humberto Garcia para ser utilizada nas mensagens complementares

//Declaração de Lógicos
Local lMensRet 		:= .F.							//Criado por Willer, imprimir retenções DANFE/xml
Local lQuery    	:= .F.
Local lCalSol		:= .F.
Local lOk			:= .T.
...
/***************************************
*** Trecho Adicionado por Luiz (DQZ) ***
***************************************/
Private cXMarca   		:= ""
Private cXNumero  		:= ""
Private cXHawb    		:= ""
...
/************************************************
*** Final do trecho Adicionado por Luiz (DQZ) ***
************************************************/
...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				If !Empty(SF2->F2_TRANSP)
					...
					/*****************************************************************************************
					***     Trecho Adicionado por Humberto Garcia para qua a placa do veículo seja buscada ***
					***     no pedido de vendas //corrigido P12 Willer indice 11                                                           ***
					*****************************************************************************************/

					dbSelectArea("SC5")
					dbSetOrder(11)
					MsSeek(xFilial("SC5")+SF2->F2_DOC+SF2->F2_SERIE)
					If !Empty(SC5->C5_X_VEIC)								
						aadd(aVeiculo,SC5->C5_X_VEIC)
						aadd(aVeiculo,SC5->C5_X_UFV)
						aadd(aVeiculo,"")//RNTC
					EndIf
					...
					While !Eof() .And. xFilial("SE1") == (cAliasSE1)->E1_FILIAL .And.;
						...
						SF2->F2_DOC == (cAliasSE1)->E1_NUM     
							If (cAliasSE1)->E1_TIPO = MVNOTAFIS .OR. (cAliasSE1)->E1_TIPO = 'DP' .OR. ((Alltrim((cAliasSE1)->E1_ORIGEM) $ 'LOJA701|FATA701|LOJA010') .AND. (cAliasSE1)->E1_TIPO $ cWhere)
								...
								DbSelectArea('SA1')//Alterado por Willer, notas desconto PIS/COFINS retencao
								If cValLiqS == "2" .And. Empty(SA1->A1_RECPIS) .Or. SA1->A1_RECPIS == 'N'
									aadd(aDupl,{(cAliasSE1)->E1_PREFIXO+(cAliasSE1)->E1_NUM+(cAliasSE1)->E1_PARCELA,(cAliasSE1)->E1_VENCORI,nValDupl})			
								ElseIf cValLiqS == "2" .And. SA1->A1_RECPIS == 'S' .And. SA1->A1_RECCOFI == 'S' .And. (cAliasSE1)->E1_COFINS > 0 .And. !SA1->A1_COD $ cCliTakat 
									...
									lMensRet := .T.
								ElseIf cValLiqS == "2" .And. SA1->A1_RECPIS == 'S' .And. SA1->A1_RECCOFI == 'S' .And. (cAliasSE1)->E1_COFINS > 0 .And. SA1->A1_COD $ cCliTakat
									//aadd(aDupl,{(cAliasSE1)->E1_PREFIXO+(cAliasSE1)->E1_NUM+(cAliasSE1)->E1_PARCELA,(cAliasSE1)->E1_VENCORI,nValDupl})--retirado conforme solicitação financeiro-reduz pis/cofins so na nf
									aadd(aDupl,{(cAliasSE1)->E1_PREFIXO+(cAliasSE1)->E1_NUM+(cAliasSE1)->E1_PARCELA,(cAliasSE1)->E1_VENCORI;
									,(nValDupl-(cAliasSE1)->E1_PIS-(cAliasSE1)->E1_COFINS-(cAliasSE1)->E1_CSLL-(cAliasSE1)->E1_INSS)-(cAliasSE1)->E1_IRRF-(cAliasSE1)->E1_ISS})	
									lMensRet := .T.
								Else
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
				...
				/*****************************************************************************************
				***     Trecho Adicionado por Humberto Garcia para Tratamento das mensagens que são    ***
				***     informadas no pedido de venda e numero do pedido do cliente e nosso pedido     ***
				*****************************************************************************************/
				// Informo o Numero do pedido de compras do Cliente
				If !Empty(Alltrim(SC6->C6_PEDCLI))
					If !AllTrim(SC6->C6_PEDCLI) $ cMensCli
						cMensCli+= "Seu Pedido: " + AllTrim(SC6->C6_PEDCLI)+ space(10)
					Endif
				Endif
				// Informo o Numero do Pedido de Vendas da Midori
				If !Empty(Alltrim(SC5->C5_NUM))
					If !AllTrim(SC5->C5_NUM) $ cMensCli
						cMensCli += "Nosso Pedido: " + Alltrim(SC5->C5_NUM)+";"
					Endif
				Endif
				// Informo as mensagens colocadas Manualmente
						If !AllTrim(SC5->C5_MENNOTA) $ cMensCli  
							...
							*///Comentado Willer ajustar as msgs
							EndIf
							If !Empty(SC5->C5_MENPAD) .And. !AllTrim(FORMULA(SC5->C5_MENPAD)) $ cMensCli
								cMensCli += AllTrim(FORMULA(SC5->C5_MENPAD))
							EndIf
							If !Empty(SC5->C5_MENPAD2) .And. !AllTrim(FORMULA(SC5->C5_MENPAD2)) $ cMensCli//Adicionado por solicitacao Rodrigo/Fiscal
								cMensCli += AllTrim(FORMULA(SC5->C5_MENPAD2))
							EndIf
							If !Empty(SC5->C5_MENPAD3) .And. !AllTrim(FORMULA(SC5->C5_MENPAD3)) $ cMensCli//Adicionado por solicitacao Rodrigo/Fiscal
								cMensCli += AllTrim(FORMULA(SC5->C5_MENPAD3))
							EndIf
							If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
								cMensCli += " "
							EndIf
							cMensCli += AllTrim(SC5->C5_MENNOTA)+" "+AllTrim(SC5->C5_MENNOT2) //+" "+AllTrim(SC5->C5_MENNOT2) foi adicionado por anesio
																							//porque nao estava saindo na nota a observacao 02
						EndIf
					...
					If lMensRet//trecho incluido por Willer para sair na DANFE retenção TAKATA -- tratado valor fatura
						dbSelectArea("SE1")
						dbSetOrder(1)
						If DbSeek(xFilial("SE1")+SF2->F2_PREFIXO+SF2->F2_DOC+" "+"NF")
							dbSelectArea("SED")
							dbSetOrder(1)
							DbSeek(xFilial("SED")+SE1->E1_NATUREZ)
							cMensCli += ";"+"***Impostos Retidos***PIS |Aliq "+Alltrim(Transform(SED->ED_PERCPIS,"@E 999,999,999.99"))+"% | Valor R$ " +Alltrim(Transform(SE1->E1_PIS,"@E 999,999,999.99"))
							cMensCli += " - Cofins |Aliq "+Alltrim(Transform(SED->ED_PERCCOF,"@E 999,999,999.99"))+"% | Valor R$ " +Alltrim(Transform(SE1->E1_COFINS,"@E 999,999,999.99"))+";"	
							lMensRet := .F.
						EndIf
					EndIf	
						
					/******************************************************************************************
					***     Final Trecho Adicionado por Humberto Garcia p/ Tratamento das mensagens que são ***
					***     informadas no pedido de venda e numero do pedido do cliente e nosso pedido      ***
					******************************************************************************************/
				
				
					/*****************************************************************************************
					***     Trecho Adicionado por Humberto Garcia para qua as mensagens sejam buscadas     ***
					***     no cadastro de TES                                                             ***
					*****************************************************************************************/
					SF4->(MsSeek(xFilial("SF4")+SC6->C6_TES))
					If !Empty(SF4->F4_MENSAG1)
						SM4->(MsSeek(xFilial("SM4")+SF4->F4_MENSAG1))
						If !Alltrim(SM4->M4_CODIGO) $ cCodTemp
							cCodTemp  += SM4->M4_CODIGO +"/"
							SF4->(MsSeek(xFilial("SF4")+SC6->C6_TES))
							cMensFis += AllTrim(FORMULA(SF4->F4_MENSAG1))+";" //CHR(10)+CHR(13)
						EndIf
					EndIf
					If !Empty(SF4->F4_MENSAG2)
						SM4->(MsSeek(xFilial("SM4")+SF4->F4_MENSAG2))
						If !Alltrim(SM4->M4_CODIGO) $ cCodTemp
							cCodTemp  += SM4->M4_CODIGO +"/"
							SF4->(MsSeek(xFilial("SF4")+SC6->C6_TES))
							cMensFis += AllTrim(FORMULA(SF4->F4_MENSAG2))+";" //CHR(10)+CHR(13)
						EndIf
					EndIf
					If !Empty(SF4->F4_MENSAG3)
						SM4->(MsSeek(xFilial("SM4")+SF4->F4_MENSAG3))
						If !Alltrim(SM4->M4_CODIGO) $ cCodTemp
							cCodTemp  += SM4->M4_CODIGO +"/"
							SF4->(MsSeek(xFilial("SF4")+SC6->C6_TES))
							cMensFis += AllTrim(FORMULA(SF4->F4_MENSAG3))+";" //CHR(10)+CHR(13)
						EndIf
					EndIf
					If !Empty(SF4->F4_MENSAG4)
						SM4->(MsSeek(xFilial("SM4")+SF4->F4_MENSAG4))
						If !Alltrim(SM4->M4_CODIGO) $ cCodTemp
							cCodTemp  += SM4->M4_CODIGO +"/"
							SF4->(MsSeek(xFilial("SF4")+SC6->C6_TES))
							cMensFis += AllTrim(FORMULA(SF4->F4_MENSAG4))+";" //CHR(10)+CHR(13)
						EndIf
					EndIf
					/*******************************************************************************************
					***  Final do trecho Adicionado por Humberto Garcia para que as mensagens sejam buscadas ***
					***  no caddastro de TES                                                                 ***
					*******************************************************************************************/
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							IIF(!Empty(SB5->B5_CEME),SB5->B5_CEME,cDescProd),;  //NAO ALTERAR - DESC CIENTIFICA PRODUTO	--Substitui a variável: cDescProd,;
...
Else
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			/********************************************************************************
			**** Trecho Adicionado por Luiz (DQZ) para tratamento das notas de importação ***
			********************************************************************************/
			//Luiz 15/07/10 - Para a impressão da NF de Importação
			OldAreaDanfe := GetArea()
			/*If !Empty(SF1->F1_HAWB) .AND. SF1->F1_TIPO == "C" // Luiz
				OldF1Area := GetArea()
				cxNfOri  := Posicione("SD1",1,xFilial("SD1")+SF1->F1_DOC+SF1->F1_SERIE,"D1_NFORI")
				cxNfOri  += " / " + Posicione("SD1",1,xFilial("SD1")+SF1->F1_DOC+SF1->F1_SERIE,"D1_SERIORI")
				cXDI     := Transform(Posicione("SW6",1,xFilial("SW6")+SF1->F1_HAWB,"W6_DI_NUM"),"@R 99/9999999-9")
				If Empty(cXHawb) .OR. SF1->F1_HAWB <> cXHawb
					cXDI     := Transform(Posicione("SW6",1,xFilial("SW6")+SF1->F1_HAWB,"W6_DI_NUM"),"@R 99/9999999-9")
				EndIf
				cXHawb   := SF1->F1_HAWB
				RestArea(OldF1Area)
			Endif */
			If !Empty(SF1->F1_HAWB) .AND. SF1->F1_TIPO == "N" // Luiz
				//lxPassou := Iif(!Empty(cXHawb) .AND. SF1->F1_HAWB == cXHawb,.T.,.F.)
				If Empty(cXHawb) .OR. SF1->F1_HAWB <> cXHawb
					cXDI     := Transform(Posicione("SW6",1,xFilial("SW6")+SF1->F1_HAWB,"W6_DI_NUM"),"@R 99/9999999-9")
				EndIf
				cXHawb   := SF1->F1_HAWB
			...
			_hawb    := SF1->F1_HAWB
			_wndoc   := SF1->F1_DOC        // Numero
			_wnserie := SF1->F1_SERIE      // Serie
			
			
			//**************
			AchaImp() //// Acha informacoes de Importaçoa para a Impressao
			//**************
			
			//* Mensagens para Importação - Luiz 15/07/2010
			//*****************************************************************************
			//**** 23/07/2010 - Melhoria Alteração - Luiz
			cxCFOImp := ""
			cD1CFO := Posicione("SD1",1,xFilial("SD1")+SF1->F1_DOC+SF1->F1_SERIE+SF1->F1_FORNECE+SF1->F1_LOJA,"D1_CF")
			cxCFOImp := alltrim(cD1CFO)
			//*****************************************************************************
			If !Empty(cXHawb) .and. SF1->F1_TIPO == "N"
				/*If Alltrim(cxCFOImp) == "3127"
					cMensCli += "Proc " +Alltrim(cXHawb)
					cMensCli += " - DI "   +Alltrim(cXDI)+";"
					cMensCli += "ICMS Isento conf art 6o parte geral item 64 anexo 1 Dec 43080/02"+";"
					cMensCli += "II/IPI suspenso conf Lei 8402/92 - PIS/COFINS suspenso conf Art 14.10865/04"+";"
					If _wdCapat > 0
						cMensCli += "Capatazias R$ "   +Alltrim(Transform(_wdCapat,"@r 999,999.99"))+";"
					Endif
				Else*///Removido conforme solicitação Fiscal Marcelo 15/10/15 - Willer
				cMensCli += "Proc "     +Alltrim(cXHawb)
				cMensCli += " - DI "       +Alltrim(cXDI)+";"
				cMensCli += "II R$ "   +Alltrim(Transform(_wniival,"@r 999,999.99"))
				cMensCli += " - PIS R$ "   +Alltrim(Transform(_wnvlrpis,"@r 999,999.99"))
				cMensCli += " - COF R$ "+Alltrim(Transform(_wnvlrcof,"@r 999,999.99"))+";"
				If _wdCapat > 0
						cMensCli += "Capatazias R$ "   +Alltrim(Transform(_wdCapat,"@r 999,999.99"))+";"
				Endif
				//EndIf
				/*************Informações dos Containers*******xSJDLacre**/
				
				cMensCli += xSJDLacre(cFilAnt,cXHawb)
			...
			If  SF1->F1_TIPO == "N" .and. SF1->F1_EST == "EX"// Willer HDI 003475
				cMensCli += "O produto está adequadamente acondicionado para suportar"+";"
				cMensCli += "os riscos de carregamento, descarregamento, transbordo e"+";"
				cMensCli += "transporte de acordo com a lei vigente."+";"
			EndIf
			
			/*If SF1->F1_TIPO == "N" .and. SF1->F1_EST == "EX"// Willer HDI 003779
				cXDescri := Posicione("SW2",1,xFilial("SW2")+SD1->D1_PEDIDO,"W2_DES_IPI")
				cMensCli += Alltrim(cXDescri)+";"
			...
			RestArea(OldAreaDanfe)
			/**********************************************************************************
			*** Final do Trecho adicionado por Luiz para tratamento das notas de importação ***
			**********************************************************************************/
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				*******************************************************************************/
				...
				//*****************************************************************************
				cxCFOImp := ""
				cxCFOImp := alltrim((cAliasSD1)->D1_CF)
				/*****************************************************************************
				** Buscando Ato Concessório - Cadastro de Li Easy Import - Luiz 22/07/2010
				*****************************************************************************/
				_cAtoCons := ""
				If !Empty(SF1->F1_HAWB) .AND. SF1->F1_TIPO == "N" .and. !Empty(cxCFOImp)
					_OldArea_Ato := GetArea()
					_cPGINum := Posicione("SW7",1,xFilial("SW7")+SF1->F1_HAWB,"W7_PGI_NUM")
					DbSelectArea("SW5")
					DbSetOrder(1)
					DbSeek(xFilial("SW5")+_cPGINum)
					If !Eof()
						While SW5->W5_PGI_NUM == _cPGINum .and. !Eof()
							If !Empty(SW5->W5_ATO_CON) .and. SW5->W5_COD_I == (cAliasSD1)->D1_COD
								_AtoMasc := Transform(SW5->W5_ATO_CON,"@R 9999-99-999999-9")+"/"+AllTrim(SW5->W5_SEQAC) // 23/07/2010 - Melhoria Alteração Luiz
								_cAtoCons := " - AC " + _AtoMasc
								Exit
							Else
								DbSkip()
							Endif
						EndDo
					Endif
					RestArea(_OldArea_Ato)
				Endif
				
				/****************************************************************************************
				*** Final do trecho adicionado por Luiz (DQZ) para tratamento das notas de importação ***
				*****************************************************************************************/
				...
				/*****************************************************************************************
				***     Trecho Adicionado por Humberto Garcia para correção do PIS quando o fornecedor ***
				***     for estrangeiro (Corrigido por Rogerio Nunes)                                  ***
				*****************************************************************************************/
				IF SA2->A2_EST == 'EX'
					aadd(aPisAlqZ , {'01'} )        //aadd( aPisAlqZ , '01' )
					aadd(aCofAlqZ , {'01'} )		 //aadd(aCofAlqZ  , '01' )
				ENDIF
				
				/***********************************************************************************************
				***  Final do trecho Adicionado por Humberto Garcia para correção do PIS quando o fornecedor ***
				***  for estrangeiro (Corrigido por Rogerio Nunes)                                           ***
				***********************************************************************************************/
				If SF4->(FieldPos("F4_CSOSN"))>0
					aTail(aCsosn):= SF4->F4_CSOSN
...
If  !lIssQn    
	...
	If Len(aIPI)>0 .And. !(cIpiCst $ "05-55-02-52-04-54") //INCLUIDO POR WILLER - VALIDACAO DRAWBACK REJEICAO 388
		cString += '<imposto>'
...
/********************************************************************
** Função criada por Luiz para tratamento das notas de importação ***
** Acha Inf Impost Import - Luiz
********************************************************************/
Static Function AchaImp()

Store 0 to _wnvlrpis,_wnvlrcof,_wniival, _wdCapat
...
DbSelectArea("SWD")
DbSetOrder(1)
Dbseek(xFilial("SWD")+_hawb+"408")
//Dbseek(xFilial("SWD")+_hawb+"436")
If !eof()
	If SF1->F1_TIPO == "N"
		_wdCapat := SWD->WD_VALOR_R
	Endif
Endif
...
/*---------------------------------------------------------
Funcao:  xSJDLacre()  |Autor: AOliveira  |Data: 10-01-2012
-----------------------------------------------------------
Descr.: Rotina tem como Objetivo retorno os Nro de lacres
dos containeres.
-----------------------------------------------------------
Param.: 01 - cFil   -> Nro da Filial
02 - cPross -> Nro do Processo
---------------------------------------------------------*/
Static Function xSJDLacre(cFil,cPross)
Local cRet		:= ""
Local cTipoCont := ""
Local cTipo 	:= ""
Local _aRetArea := GetArea()

DbSelectArea('SJD')
SJD->(DbSetOrder(1))
SJD->(DBGotop())
SJD->(DBSeek(xFilial("SJD")+cPross))
While SJD->(!Eof()) .And. SJD->(JD_FILIAL+JD_HAWB) == xFilial("SJD")+cPross
	If !Empty(SJD->JD_TIPO_CT)
		If !Empty(SJD->JD_LACRE)
			DO Case
				Case SJD->JD_TIPO_CT == "1"
					cTipo := "Container 20' No.: "
				Case SJD->JD_TIPO_CT == "2"
					cTipo := "Container 40' No.: "
				Case SJD->JD_TIPO_CT == "3"
					cTipo := "Container 40' HC No.: "
				Case SJD->JD_TIPO_CT == "4"
					cTipo := "Outros No.: "
			EndCase
			
			cRet += cTipo + Alltrim(SJD->JD_CONTAIN) +"- Lacre No.:" + Alltrim(SJD->JD_LACRE) +" / "
