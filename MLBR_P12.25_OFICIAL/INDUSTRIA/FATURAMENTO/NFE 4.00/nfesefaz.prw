...
local cEstado       := ""
local cICMSZFM      := ""
Local cValLiqS		:= SuperGetMv("MV_BR10925", ,"2")		//Adicionado por Willer tratar reten��es de venda
Local cCliTakat		:= SuperGetMv("MA_RETTAKA", ,"000008")		//Adicionado por Willer tratar reten��es de venda
...
Local nCountIT	:= 0
Local nDesVrIcms:= 0
Local cCodTemp   	:= "" // Vari�vel criada por Humberto Garcia para ser utilizada nas mensagens complementares

//Declara��o de L�gicos
Local lMensRet 		:= .F.							//Criado por Willer, imprimir reten��es DANFE/xml
Local lQuery    	:= .F.
Local lCalSol		:= .F.
Local lOk			:= .T.
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				If !Empty(SF2->F2_TRANSP)
					...
					/*****************************************************************************************
					***     Trecho Adicionado por Humberto Garcia para qua a placa do ve�culo seja buscada ***
						...
						SF2->F2_DOC == (cAliasSE1)->E1_NUM     
							If (cAliasSE1)->E1_TIPO = MVNOTAFIS .OR. (cAliasSE1)->E1_TIPO = 'DP' .OR. ((Alltrim((cAliasSE1)->E1_ORIGEM) $ 'LOJA701|FATA701|LOJA010') .AND. (cAliasSE1)->E1_TIPO $ cWhere)
								...
								ElseIf cValLiqS == "2" .And. SA1->A1_RECPIS == 'S' .And. SA1->A1_RECCOFI == 'S' .And. (cAliasSE1)->E1_COFINS > 0 .And. !SA1->A1_COD $ cCliTakat 
									...
									lMensRet := .T.
								ElseIf cValLiqS == "2" .And. SA1->A1_RECPIS == 'S' .And. SA1->A1_RECCOFI == 'S' .And. (cAliasSE1)->E1_COFINS > 0 .And. SA1->A1_COD $ cCliTakat
									//aadd(aDupl,{(cAliasSE1)->E1_PREFIXO+(cAliasSE1)->E1_NUM+(cAliasSE1)->E1_PARCELA,(cAliasSE1)->E1_VENCORI,nValDupl})--retirado conforme solicita��o financeiro-reduz pis/cofins so na nf
									aadd(aDupl,{(cAliasSE1)->E1_PREFIXO+(cAliasSE1)->E1_NUM+(cAliasSE1)->E1_PARCELA,(cAliasSE1)->E1_VENCORI;
									,(nValDupl-(cAliasSE1)->E1_PIS-(cAliasSE1)->E1_COFINS-(cAliasSE1)->E1_CSLL-(cAliasSE1)->E1_INSS)-(cAliasSE1)->E1_IRRF-(cAliasSE1)->E1_ISS})	
									lMensRet := .T.
								Else
	          ...
	          // Posiciono na TES do cupom fiscal para pegar a natureza de opera��o da nf sobre cupom
				...
				/*****************************************************************************************
				***     Trecho Adicionado por Humberto Garcia para Tratamento das mensagens que s�o    ***
				...
				*****************************************************************************************/
				...
				// Informo as mensagens colocadas Manualmente
					...
					If lMensRet//trecho incluido por Willer para sair na DANFE reten��o TAKATA -- tratado valor fatura
						dbSelectArea("SE1")
						dbSetOrder(1)
					...
					/******************************************************************************************
					***     Final Trecho Adicionado por Humberto Garcia p/ Tratamento das mensagens que s�o ***
					***     informadas no pedido de venda e numero do pedido do cliente e nosso pedido      ***
					******************************************************************************************/
					...
					*******************************************************************************************/
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							IIF(!Empty(SB5->B5_CEME),SB5->B5_CEME,cDescProd),;  //NAO ALTERAR - DESC CIENTIFICA PRODUTO	--Substitui a vari�vel: cDescProd,;
...
Else
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			/********************************************************************************
			**** Trecho Adicionado por Luiz (DQZ) para tratamento das notas de importa��o ***
			********************************************************************************/
			//Luiz 15/07/10 - Para a impress�o da NF de Importa��o
			...
			//**************
			AchaImp() //// Acha informacoes de Importa�oa para a Impressao
			//**************
			
			//* Mensagens para Importa��o - Luiz 15/07/2010
			//*****************************************************************************
			//**** 23/07/2010 - Melhoria Altera��o - Luiz
			cxCFOImp := ""
			...
			If !Empty(cXHawb) .and. SF1->F1_TIPO == "N"
				/*If Alltrim(cxCFOImp) == "3127"
					...
					Endif
				Else*///Removido conforme solicita��o Fiscal Marcelo 15/10/15 - Willer
				cMensCli += "Proc "     +Alltrim(cXHawb)
				cMensCli += " - DI "       +Alltrim(cXDI)+";"
				...
				Endif
				//EndIf
				/*************Informa��es dos Containers*******xSJDLacre**/
			...
			If  SF1->F1_TIPO == "N" .and. SF1->F1_EST == "EX"// Willer HDI 003475
				cMensCli += "O produto est� adequadamente acondicionado para suportar"+";"
			...
			/**********************************************************************************
			*** Final do Trecho adicionado por Luiz para tratamento das notas de importa��o ***
			**********************************************************************************/
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				*******************************************************************************/
				...
				/*****************************************************************************
				** Buscando Ato Concess�rio - Cadastro de Li Easy Import - Luiz 22/07/2010
				*****************************************************************************/
				_cAtoCons := ""
					...
					If !Eof()
						While SW5->W5_PGI_NUM == _cPGINum .and. !Eof()
							If !Empty(SW5->W5_ATO_CON) .and. SW5->W5_COD_I == (cAliasSD1)->D1_COD
								_AtoMasc := Transform(SW5->W5_ATO_CON,"@R 9999-99-999999-9")+"/"+AllTrim(SW5->W5_SEQAC) // 23/07/2010 - Melhoria Altera��o Luiz
				...
				/****************************************************************************************
				*** Final do trecho adicionado por Luiz (DQZ) para tratamento das notas de importa��o ***
				*****************************************************************************************/
				...
				/*****************************************************************************************
				***     Trecho Adicionado por Humberto Garcia para corre��o do PIS quando o fornecedor ***
				...
				*****************************************************************************************/
				...
				/***********************************************************************************************
				***  Final do trecho Adicionado por Humberto Garcia para corre��o do PIS quando o fornecedor ***
				***  for estrangeiro (Corrigido por Rogerio Nunes)                                           ***
				***********************************************************************************************/
				If SF4->(FieldPos("F4_CSOSN"))>0
					aTail(aCsosn):= SF4->F4_CSOSN
...
/********************************************************************
** Fun��o criada por Luiz para tratamento das notas de importa��o ***
