...
#DEFINE MAXMENLIN  098                                                // Máximo de caracteres por linha de dados adicionais
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
aTotais := {"","","","","","","","","","",""}
aTotais[01] := Transform(Val(oTotal:_ICMSTOT:_vBC:TEXT),"@e 999,999,999.99")
aTotais[02] := Transform(Val(oTotal:_ICMSTOT:_vICMS:TEXT),"@e 9,999,999.99")
aTotais[03] := Transform(Val(oTotal:_ICMSTOT:_vBCST:TEXT),"@e 999,999,999.99")
aTotais[04] := Transform(Val(oTotal:_ICMSTOT:_vST:TEXT),"@e 9,999,999.99")
aTotais[05] := Transform(Val(oTotal:_ICMSTOT:_vProd:TEXT),"@e 9,999,999.99")
aTotais[06] := Transform(Val(oTotal:_ICMSTOT:_vFrete:TEXT),"@e 9,999,999.99")
aTotais[07] := Transform(Val(oTotal:_ICMSTOT:_vSeg:TEXT),"@e 9,999,999.99")
...
If ( MV_PAR04 == 1 )
	...
	If MsSeek(xFilial("SF1")+aNota[5]+aNota[4]+aNota[6]+aNota[7]) .And. SF1->(FieldPos("F1_FIMP"))<>0
		If SF1->F1_TIPO <> "D"
	aTotais[10] := 	Transform(Val(oTotal:_ICMSTOT:_vIPI:TEXT),"@e 9,999,999.99")
...
If Type("oTransp:_Vol")<>"U"
	...
	Else
		...
		//		Alterado por Humberto Garcia para sempre imprimir a palavra LETRA no quadro MARCA, da DANFE
		//aTransp[13] := IIf(Type("oTransp:_Vol:_Marca")=="U","",NoChar(oTransp:_Vol:_Marca:TEXT,lConverte))
		aTransp[13] := 'LETRA'
		aTransp[14] := IIf(Type("oTransp:_Vol:_nVol:TEXT")<>"U",oTransp:_Vol:_nVol:TEXT,"")
	...
	EndIf
//	aTransp[15] := strTRan(aTransp[15],".",",") // Comentado por Anesio em 19-09-2011 por nao estar saindo as casas decimais na danfe
//	aTransp[16] := strTRan(aTransp[16],".",",") // Comentado por Anesio em 19-09-2011 por nao estar saindo as casas decimais na danfe
EndIf
...
For nZ := 1 To nLenDet
	...
	//AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1],TamSX3("D2_PRCVEN")[2]))))       //WILLER 2/2/12 NT2010.005
//	AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1],2))))                            //AOliveira 19-09-11 //Alterado para 6 casas decimais por Anesio em 02/10/2012
	AADD(aAux, AllTrim(TransForm(nVUnit,"@E 999,999.999999")))
	AADD(aAux, AllTrim(TransForm(nVTotal,TM(nVTotal,TamSX3("D2_TOTAL")[1],2)))) //TamSX3("D2_TOTAL")[2]))))
...
If Type("oIdent:_tpAmb:TEXT")<>"U" .And. oIdent:_tpAmb:TEXT=="2"
	...
	While !Empty(cAux)
		If (At(";",cAux) > 0) .And. ((At(";",cAux) <= (MAXMENLIN-1)))          // Trecho adicionado por Humberto Garcia para possibilitar a quebra da linha nas
			aadd(aMensagem,SubStr(cAux,1,(AT(";",cAux)-1)))                    // informações adicionais da DANFE, no momento da impressão.
			cAux := SubStr(cAux,(AT(";",cAux)+1))                              // Essa logica permanece a mesma nos quadros abaixo, para contemplar as diferentes
		Else                                                                   // situações (normal, contingencia, contingencia SCAN, etc)
...
nForTo		:=	Len(cStrAux)/20
nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
For nX := 1 To nForTo
	oDanfe:Say(nLinCalc,098,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*20)+1),20), oFont12N:oFont )
	nLinCalc+=10
Next nX
...
If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
	cStrAux		:=	"Compl - "+AllTrim(NoChar(oEmitente:_EnderEmit:_xCpl:TEXT,lConverte))
	...
	nLinCalc+=10
	oDanfe:Say(nLinCalc,098, "TEL - "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
Else
...
If nfolha==1
	...
	Else
		oDanfe:SayBitmap(049,006,cLogo,075,076)
	EndIF
Endif
...
//oDanfe:Box(721,000,865,351)  Alterado por Humberto Garcia para ajuste no quadro de informações complementares
oDanfe:Box(721,000,865,395)
...
For nX := 1 To Min(nLenMensagens, MAXMSG)
	oDanfe:Say(nLin,002,aMensagem[nX],oFont07:oFont)
	nLin:= nLin+10
Next nX
nMensagem := nX
/*******************************************************************************************
****** Trecho Alterado por Humberto Garcia para ajustar o quadro RESERVADO AO FISCO   ******
****** para que a informação das mensagens não sejam sobrepostas                      ******
*******************************************************************************************/

/*oDanfe:Box(721,350,865,603)
oDanfe:Say(729,352,"RESERVADO AO FISCO",oFont08N:oFont)*/

oDanfe:Box(721,395,865,603)
oDanfe:Say(729,397,"RESERVADO AO FISCO",oFont08N:oFont)

/****************************************************************************************************
****** Final do trecho Alterado por Humberto Garcia para ajustar o quadro RESERVADO AO FISCO   ******
****** para que a informação das mensagens não sejam sobrepostas                               ******
****************************************************************************************************/
...
For nY := 1 To nLenItens
		...
		If nL > nMaxItemP2
			...
			Else
			   //	oDanfe:SayBitmap(000+nLinhavers,000,cLogo,095,096)
			    oDanfe:SayBitmap(004,006,cLogo,075,076)
			EndIf
			...
			oDanfe:Say(171+nLinhavers, nAuxH + 2, "V.UNIT", oFont08N:oFont)
			nAuxH += aTamCol[8]
	...
	If	nL > Iif( (nfolha-1)%2==0 .And. MV_PAR05==1,MAXITEMP3,nMaxItemP2)
		...
		oDanfe:Say(171, nAuxH + 2, "V.UNIT", oFont08N:oFont)
		nAuxH += aTamCol[8]
