...
Programa : BolTokyo
-----------------------------------------------------------------------------
Descricao: Boleto de Cobranca - Banco Tokyo - Identico Bradesco
-----------------------------------------------------------------------------
Especifico para Midori Atlantica
-----------------------------------------------------------------------------
Analista -> Willer Trindade
-----------------------------------------------------------------------------
*/
User Function BolTokyo()
Local wnrel
Local Tamanho		:= "G"
Local titulo		:= "Boleto de Cobrança - Banco de Tokyo-Mitsubishi"
Local cDesc1		:= "Este programa imprime os boletos de"
Local cDesc2		:= "cobranca bancaria direta de acordo com os parâmetros informados"
Local cDesc3		:= "Cobrança Banco de Tokyo-Mitsubishi - Carteira - 19"
Local cDesc4		:= ""
Local cDesc5		:= ""
Local aSays     	:= {}, aButtons := {}, nOpca := 0

Private nomeprog 	:= "BolTokyo"
Private nLastKey 	:= 0
Private cPerg    	:= "BOL456"
Private oPrint

Private oFontAr8	:= TFont():New("Segoe UI", 8, 8,,.F.,,,,.T.,.F.)	//Normal
Private oFontAr9	:= TFont():New("Segoe UI", 9, 9,,.F.,,,,.T.,.F.)	//Normal
Private oFontAr10	:= TFont():New("Segoe UI",10,10,,.F.,,,,.T.,.F.)	//Normal
Private oFontAr14	:= TFont():New("Segoe UI",14,14,,.F.,,,,.T.,.F.)	//Normal
Private oFontAr16	:= TFont():New("Segoe UI",16,16,,.F.,,,,.T.,.F.)	//Normal

Private oFontAr8n	:= TFont():New("Segoe UI", 8, 8,,.T.,,,,.T.,.F.)	//Negrito
Private oFontAr9n	:= TFont():New("Segoe UI", 9, 9,,.T.,,,,.T.,.F.)	//Negrito
Private oFontAr10n	:= TFont():New("Segoe UI",10,10,,.T.,,,,.T.,.F.)	//Negrito
Private oFontAr11n	:= TFont():New("Segoe UI",11,11,,.T.,,,,.T.,.F.)	//Negrito
Private oFontAr12n	:= TFont():New("Segoe UI",12,12,,.T.,,,,.T.,.F.)	//Negrito
Private oFontAr14n	:= TFont():New("Segoe UI",14,14,,.T.,,,,.T.,.F.)	//Negrito
Private oFontAr16n	:= TFont():New("Segoe UI",16,16,,.T.,,,,.T.,.F.)	//Negrito

Private oFont3 		:= TFont():New("Segoe UI",,14,,.T.,,,,.T.,.F. )	//Negrito
...
RptStatus({|lEnd| RProcBol(@lEnd,wnRel,cString)},Titulo)
...
----------------------------------------------------------------------*/
Static Function RProcBol()
Private cNReduz  := "Banco de Tokyo-Mitsubishi"
Private cLocalPag:= "Pagável Preferencialmente na Rede Bradesco"
Private cUsoBco  := "8650"
Private cCIP     := "000"
Private cSacador := Alltrim(SM0->M0_NOMECOM), cInstr1 := "", cInstr2 := ""
Private cIdBco   := "237-2"
Private cMoeda   := "9"
Private cCarteira:= "19"
//Private cDescCrt := "COBRANCA REGISTRADA"
Private cAgencia := "2372" 	//SA6->(A6_AGENCIA+A6_X_DIGAG)
Private cConta   := "0006739" //SA6->A6_NUMCON
Private cDvCC    := "3" //SA6->A6_X_DIGCT
Private cCodCed  := ""
Private cNN      := "", cDvNN := ""
Private cBanco   := "237"
...
//cConta    := Substr(cSA6Conta,1,7)	//Alltrim(TRB->A6_NUMCON)
...
While TMP->(!Eof())
	...
	While TMP->(!Eof()) .And. TMP->(E1_CLIENTE + E1_LOJA) == SA1->(A1_COD + A1_LOJA)
		...
		Codigo do Cedente
		----------------------------------*/
		cCodCed := Alltrim(cAgencia) +" / " + Alltrim(cConta) + "-" + Alltrim(cDvCC)			//Alltrim(TRB->A6_AGENCIA)+"-"+Alltrim(TRB->A6_X_DIGAG)
...
//			cNN      := StrZero(nNumfaix,11)
		...
		cValor   := STRZERO(TMP->E1_SALDO * 100,10)
...
----------------------------------------------------------------------*/
Static Function ImpBolTok()
Local nLin    := 220
Local nColBIni:= 120
Local nColBFim:= 2300
Local cCgc := "60398914000184" //If(Empty(SM0->M0_CGC),"00000000000000",SM0->M0_CGC)
Local cNome:= "Banco de Tokyo-Mitsubishi UFJ Brasil S/A."		//If(Empty(SM0->M0_NOMECOM),"NOME DO CEDENTE",SM0->M0_NOMECOM)
Local cDataEmi := GravaData(StoD(TMP->E1_EMISSAO),.f.,5)
Local cDataVenc:= GravaData(StoD(TMP->E1_VENCTO),.f.,5)
Local cDataProc:= GravaData(dDataBase,.f.,5)	//ddmmaaaa
Local nCnt
...
For nCnt := 1 To 2
	...
	oPrint:Say(nLin,120,"Cedente",oFontAr8)
	oPrint:Say(nLin,1660,"Agência/Código Cedente",oFontAr8)
	...
	oPrint:Say(nLin,1730,Transform(TMP->E1_SALDO,"@E 999,999,999.99"),oFontAr10n) // Valor do documento
	nLin += 40	//2660
	...
	If Mv_Par09 > 0    //A PERGUNTA TEM Q ESTAR PREENCHIDA PARA SAIR VALOR MULTA - WILLER
		...
		nMulta := (TMP->E1_SALDO * (Mv_Par09 / 100))
		oPrint:Say(nLin, 120,"Após vencimento cobrar Multa de R$ "+Alltrim(Transform(nMulta,"@E 999,999.99")),oFontAr8) //WILLER HDI 004612
	...
	//Rotina para imprimir o desconto de imposto retido da TAKATA - Anesio: 20/09/2012
		...
		if IMPRET->E1_VALOR > 0		
			oPrint:Say(nLin+30, 120,"CONCEDER DESCONTO DE R$ "+Alltrim(Transform(IMPRET->E1_VALOR,"@E 999,999.99"))+' REF.PIS/COFINS RETIDO' ,oFontAr8)
		endif
	...
	oPrint:Line(nLin,nColbIni,nLin,nColbFim)
	oPrint:Say(nLin,nColbIni,"Sacado ",oFontAr8)
	nLin += 40	//3050
	...
	nLin += 50	//3180
	oPrint:Say(nLin,nColbIni,"Sacador/Avalista: "+Alltrim(cSacador)+ Space(10)+"CNPJ "+Transform(cCgc,"@R 99.999.999/9999-99"),oFontAr10)
	//	oPrint:Say(nLin,1660,"Código de Baixa: ",oFontAr8)
	nLin += 40	//3210
	oPrint:Line(nLin+5,nColbIni,nLin+5,nColbFim)
	If nCnt == 1
		oPrint:Say(nLin+10,1460,"Autenticação Mecânica / RECIBO DO SACADO",oFontAr8)
	EndIf
	/*
	--------------------------------
...
If Len(aTitulos) > 0
	For nCnt := 1 To Len(aTitulos)
		...
		SE1->(MsSeek(cSE1Fil + xPrefixo + xNumero + xParcela + xTipo)) //AOliveira 03-02-12
