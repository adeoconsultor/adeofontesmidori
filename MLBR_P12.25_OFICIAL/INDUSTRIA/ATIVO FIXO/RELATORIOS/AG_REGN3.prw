...
incproc('Atualizando temp ...Reg '+cValToChar(nCount)+' de '+cValToChar(nReg))
	...
	aConteud[len(aConteud),4] := TMPN3->N3_BAIXA 
	aConteud[len(aConteud),5] := TMPN3->N3_HISTOR 
	aConteud[len(aConteud),6] := "'"+TMPN3->N3_CCONTAB
	aConteud[len(aConteud),7] := TMPN3->N3_CUSTBEM
	aConteud[len(aConteud),8] := TMPN3->N3_CDEPREC 	
	aConteud[len(aConteud),9] := TMPN3->N3_VORIG1 
	aConteud[len(aConteud),10] := TMPN3->N3_VRDMES1
	aConteud[len(aConteud),11] := TMPN3->N3_VRDACM1 
	aConteud[len(aConteud),12]:= TMPN3->N3_AQUISIC
	aConteud[Len(aConteud),13] := TMPN3->N1_STATUS
/*
...
//   if Alltrim(SX3->X3_CAMPO) $ 'N3_FILIAL, N1_NFISCAL, N1_NSERIE, N1_PRODUTO, N1_FORNEC, N1_LOJA, N3_CBASE,N3_BAIXA,N3_HISTOR,N3_CCONTAB,N3_CUSTBEM,N3_VRDMES1,N3_VORIG1,N3_VRDACM1,N3_AQUISIC'	
   if Alltrim(SX3->X3_CAMPO) $ 'N3_FILIAL,N3_CBASE,N3_ITEM,N3_BAIXA,N3_HISTOR,N3_CCONTAB,N3_CUSTBEM,N3_CDEPREC,N3_VRDMES1,N3_VORIG1,N3_VRDACM1,N3_AQUISIC'		
...
dbSetOrder(1)
dbSeek("SN1")
While !EOF() .And. (x3_arquivo == "SN1")

//   if Alltrim(SX3->X3_CAMPO) $ 'N3_FILIAL, N1_NFISCAL, N1_NSERIE, N1_PRODUTO, N1_FORNEC, N1_LOJA, N3_CBASE,N3_BAIXA,N3_HISTOR,N3_CCONTAB,N3_CUSTBEM,N3_VRDMES1,N3_VORIG1,N3_VRDACM1,N3_AQUISIC'	
   if Alltrim(SX3->X3_CAMPO) $ 'N1_STATUS'		
		cCampos := cCampos+SX3->X3_CAMPO+","	
		aAdd( aTitulos, {SX3->X3_TITULO} )
		aAdd(aCampos, {ALLTRIM(SX3->X3_CAMPO), SX3->X3_TIPO})
	endif
...
//	Alert('Total de Registros -> '+cValToChar(TMPCT->NREG))
	...
	cQuery := " SELECT N3_FILIAL,N3_CBASE,N3_ITEM,N3_BAIXA,N3_HISTOR,N3_CCONTAB,N3_CUSTBEM,N3_CDEPREC, "
	cQuery += " N3_VRDMES1,N3_VORIG1,N3_VRDACM1,N3_AQUISIC, N1_STATUS "
	cQuery += " FROM SN3010 SN3, SN1010 SN1 "
	cQuery += " WHERE SN3.D_E_L_E_T_ = ' ' and SN1.D_E_L_E_T_ = ' ' "
	cQuery += " and N1_FILIAL = N3_FILIAL and N1_CBASE = N3_CBASE and N1_ITEM = N3_ITEM ORDER BY 1,2,3"

	
//	cQuery := " SELECT N3_FILIAL,N3_CBASE,N3_ITEM,N3_BAIXA,N3_HISTOR,N3_CCONTAB,N3_CUSTBEM,N3_CDEPREC,N3_VRDMES1,N3_VORIG1,N3_VRDACM1,N3_AQUISIC FROM SN3010 WHERE D_E_L_E_T_ = ' '  " 
