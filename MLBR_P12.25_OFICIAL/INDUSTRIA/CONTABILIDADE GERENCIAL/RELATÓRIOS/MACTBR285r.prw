...
±±ºPrograma  ³Ctbr285r  ºAutor  ³Claudio Servulo     º Data ³  31/05/2010 º±±
...
DEFINE SBUTTON FROM 50,160 TYPE 1  OF oDlgSem ACTION semestre() ENABLE
...
do while cArqTmp->(!eof())
	...
	cQuery := ""
	cQuery += " SELECT SUM(CT4_DEBITO-CT4_CREDIT) AS VALOR  "
	cQuery += " FROM " + RETSQLNAME("CT4")
	cQuery += " WHERE CT4_CONTA = '"+cArqTmp->CONTA+"'"
	cQuery += " AND CT4_FILIAL = '" + cFilAnt + "'"
	cQuery += " AND CT4_DATA BETWEEN '"+ DTOS(MV_PAR01) + "' AND '" + DTOS(MV_PAR02)+"'"
...
oCentroCusto:Cell("CTT_CUSTO"):SetBlock({||EntidadeCTB(Subs(cArqTmp->CONTA,1,10),,,15,.F.,cMascara,cSepara2,/*cAlias*/,/*nOrder*/,.F./*lGraf*/,/*oPrint*/,.F./*lSay*/) } )
